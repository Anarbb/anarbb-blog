...
def index():
    ...
    if "email" and "username" and "type" in session:
        if request.method == "POST":
            ...
            username_found = users.query.filter_by(username=session["username"]).first()
            posted_by_found = posts.query.filter_by(posted_by=session["username"])
            if email:
        ...
        return render_template(
            "user/index.html",
            posts=posts.query.filter_by(posted_by=session["username"])
            .order_by(desc(posts._id))
            .all(),
            email=session["email"],
